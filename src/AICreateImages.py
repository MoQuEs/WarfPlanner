...
from Init import arknights
from Utils import materials_dir, auto_labels_in_dir, overlay_image, get_random_image, \
    generate_random_string as grs, get_mats_to_generate_auto_labels, clear_auto_labels_in_dir
...
mats_data = get_mats_to_generate_auto_labels(
    arknights,
    lambda data: data["count"] < 50 or data["represented"] != "Over Represented",
    True
)
...
get_random_mat_id = lambda: choices(ids, normalized_weights)[0]
    ...
    for col in range(0, cols):
        for row in range(0, rows):
            ...
            overlay_image(
                ...
                material_image,
                (
                    sp_width + (fw_size * col) + pw_size,
                    sp_height + (fh_size * row) + ph_size
                )
            )

    imwrite(
        auto_labels_in_dir("%s-%s.%s" % (grs(), grs(), 'png')),
        base_image,
